function [g_k_t] = optSolver_JEKY_analysis(problem,method,options)
  ...
  g_k_t = [norm_g, k, totalTime]; % store the updated 'g-inf' after each iteration
  ...
  while norm_g > options.term_tol * max(norm_g_0, 1) && k < options.max_iterations
    ...
    g_k_t = [g_k_t; norm_g, k, totalTime]; 
  end
