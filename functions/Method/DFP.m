...
%
% DFP Method with Backtracking Line Search
...
function[x_new, f_new, g_new, D_new] = DFP(x, g, D, problem, method, options)
    %Compute the serach direction 
    d = -D * g;

    %Compute the step length alpha_k
    % Perform backtracking line search to determine step length
    % determine step size
    alpha_0 = method.options.alpha_0;
    tau = method.options.tau; 
    c_1_ls = method.options.c_1_ls;
    alpha_k = BLS(x, g, alpha_0, tau, c_1_ls, d, problem);     
